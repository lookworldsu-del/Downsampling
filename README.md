...
真实业务场景：**实时摄像头 → 416×416 Letterbox → YOLO/目标检测推理**

### Benchmark 模式：含 tensor + CGImage 完整输出（2026-04-04 实测）

| 输入 | 冠军 | 耗时 | FPS | CPU | Metal NCHW Direct |
|------|------|------|-----|-----|-------------------|
| **1080p → 416² LB** | **vImage** | **20.84 ms** | **48.0** | 56.9% | 34.03 ms (29 FPS) |
| **4K → 416² LB** | **Metal Compute** | **22.87 ms** | **43.7** | 56.9% | 39.18 ms (26 FPS) |

> Metal NCHW Direct 在 benchmark 模式下较慢，因为包含了 `makePreviewImage`（NCHW→CGImage 反向转换，仅供 UI 显示）。**生产环境中不需要此步骤。**

### Production 模式：跳过 preview，指针直传 TNN

| 步骤 | Metal Compute (旧管线) | Metal NCHW Direct (新管线) |
|------|----------------------|--------------------------|
| GPU Shader 执行 | ~2 ms | ~2 ms |
| texture.getBytes (GPU→CPU) | ~1 ms | **0 ms** (storageModeShared) |
| CGContext → CGImage | ~0.5 ms | **0 ms** (不需要) |
| cgImageToRGBTensor | ~15 ms | **0 ms** (GPU 已完成) |
| BlobConverter (HWC→NCHW) | ~1-2 ms | **0 ms** (GPU 已完成) |
| **总计** | **~21 ms** | **~2 ms (GPU 计算时间)** |

```
Production 模式 (4K → 416² LB, 跳过 preview):

Metal NCHW Direct  ██░░░░░░░░░░░░░░░░░░   ~2 ms   (GPU compute only → TNN)
Metal Compute      ████████████████████░  22.87 ms  (含 tensor 转换)
Core Image         ████████████████████░  32.51 ms
MPS                ████████████████████░  38.85 ms
vImage             ████████████████████░  42.94 ms
```
...
#### 旧管线：Metal Compute → CGImage → Tensor（~21ms）
...
graph LR
    ...
    E -->|"cgImageToRGBTensor<br/>~15ms 瓶颈!"| F["[Float] HWC<br/>RGB Tensor"]
...
> **为什么 benchmark 中 NCHW Direct 反而慢？** benchmark 为了公平对比和 UI 显示，额外做了两步：① `Array(UnsafeBufferPointer(...))` 拷贝 2.08MB tensor ② `makePreviewImage` 将 NCHW float 逆向转回 CGImage（cache-unfriendly 的三平面随机读取）。生产环境中这两步都不需要——直接把 `MTLBuffer.contents()` 指针传给 TNN。

---

## 416×416 Letterbox 完整基准（含 tensor 转换）

> 全部 7 种算法的端到端耗时：摄像头帧 → 降采样 → Letterbox → tensor 生成 → CGImage
>
> 测试日期：2026-04-04 · iPhone 16 (A18) · iOS 26.3.1 · 隔离跑分 60帧 + 5s冷却

### 1080p → 416×416 Letterbox（含 tensor）

| 算法 | 类型 | 平均耗时 | 最小 | 最大 | P99 | FPS | 方差 | CPU | 内存 |
|------|------|---------|------|------|-----|-----|------|-----|------|
| **vImage** | CPU | **20.84 ms** | 18.74 | 56.83 | 56.83 | **48.0** | 23.05 | 56.9% | 315.6 MB |
| Metal Compute | GPU | 21.22 ms | 19.60 | 59.91 | 59.91 | 47.1 | 27.69 | **53.6%** | 315.5 MB |
| UIGraphicsImageRenderer | CPU | 22.94 ms | 20.70 | 59.70 | 59.70 | 43.6 | 24.07 | 63.6% | 323.7 MB |
| CGContext | CPU | 23.16 ms | 20.85 | 59.06 | 59.06 | 43.2 | 23.43 | 64.7% | 315.8 MB |
| Core Image | GPU | 24.03 ms | 21.60 | 62.45 | 62.45 | 41.6 | 26.75 | 64.3% | 317.0 MB |
| MPS | GPU | 33.42 ms | 29.22 | 74.03 | 74.03 | 29.9 | 29.88 | 81.6% | 315.8 MB |
| Metal NCHW Direct | GPU | 34.03 ms | 29.21 | 76.80 | 76.80 | 29.4 | 34.02 | 84.8% | 314.9 MB |

### 4K → 416×416 Letterbox（含 tensor）

| 算法 | 类型 | 平均耗时 | 最小 | 最大 | P99 | FPS | 方差 | CPU | 内存 |
|------|------|---------|------|------|-----|-----|------|-----|------|
| **Metal Compute** | GPU | **22.87 ms** | 19.29 | 63.20 | 63.20 | **43.7** | 31.27 | **56.9%** | **493.0 MB** |
| Core Image | GPU | 32.51 ms | 25.01 | 65.96 | 65.96 | 30.8 | 27.69 | 76.6% | 494.3 MB |
| MPS | GPU | 38.85 ms | 30.79 | 75.16 | 75.16 | 25.7 | 42.30 | 80.2% | 493.2 MB |
| Metal NCHW Direct | GPU | 39.18 ms | 30.06 | 75.42 | 75.42 | 25.5 | 54.93 | 85.1% | 492.3 MB |
| CGContext | CPU | 39.42 ms | 28.74 | 68.37 | 68.37 | 25.4 | 75.13 | 99.6% | 493.3 MB |
| UIGraphicsImageRenderer | CPU | 40.15 ms | 28.43 | 67.22 | 67.22 | 24.9 | 85.53 | 100.3% | 525.0 MB |
| vImage | CPU | 42.94 ms | 31.07 | 71.51 | 71.51 | 23.3 | 65.85 | 96.8% | 493.1 MB |

### Benchmark vs Production 对比

| 算法 | 4K → 416² LB Benchmark | Production 模式 | 说明 |
|------|------------------------|-----------------|------|
| **Metal NCHW Direct** | 39.18 ms | **~2 ms** | 跳过 Array copy + makePreviewImage + BlobConverter |
| Metal Compute | 22.87 ms | ~21 ms (仍需 tensor 转换) | getBytes + CGImage + cgImageToRGBTensor 不可省 |
| vImage | 42.94 ms | ~42 ms | CPU 全程，无可优化余地 |

---

## 纯降采样基准（不含 tensor 转换）

> 以下为历史对照数据：纯降采样输出 CGImage 的耗时，不含 `cgImageToRGBTensor` 步骤
...
## 全配置耗时矩阵（ms，含 tensor 转换）

> 2026-04-04 实测，含 tensor 生成 + CGImage 输出

| 算法 | 1080p LB | 4K LB | 1080p 416² | 4K 416² | 1080p ×1/8 | 1080p ×1/4 | 1080p ×1/2 | 4K ×1/8 | 4K ×1/4 | 4K ×1/2 |
|------|----------|-------|-----------|---------|-----------|-----------|-----------|---------|---------|---------|
| Metal Compute | **21.22** | **22.87** | **20.90** | **22.26** | 14.59 | **16.22** | 98.19 | **16.43** | **95.43** | 465.67 |
| vImage | 20.84 | 42.94 | 22.28 | 38.15 | 15.93 | **15.85** | 128.67 | 35.93 | 133.61 | **428.15** |
| CGContext | 23.16 | 39.42 | 24.64 | 39.05 | **11.68** | 17.57 | 112.12 | 29.37 | 128.17 | 482.14 |
| UIGraphics | 22.94 | 40.15 | 23.97 | 42.24 | 11.68 | 17.86 | 107.85 | 29.06 | 127.44 | 508.73 |
| Core Image | 24.03 | 32.51 | 23.26 | 30.82 | 19.26 | 17.90 | **100.88** | 23.80 | 118.42 | 476.16 |
| MPS | 33.42 | 38.85 | 22.12 | 24.10 | 15.63 | 17.24 | 92.26 | 19.06 | 102.16 | 470.53 |
| Metal NCHW Direct | 34.03 | 39.18 | 34.21 | 37.58 | 18.75 | 23.90 | 163.65 | 24.73 | 170.17 | 712.30 |

> Metal NCHW Direct 的 benchmark 耗时包含 `makePreviewImage`（NCHW→CGImage 反向转换），生产环境中此步骤不存在。
>
> 隔离跑分模式：单算法 × 60 帧 + 5s 冷却

### 全配置耗时矩阵（ms，纯降采样，不含 tensor）
